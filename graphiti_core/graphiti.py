...
class Graphiti:
    ...
    async def extract_nodes_and_edges_only(
            self,
            group_id: str,
            episode: EpisodicNode,
    ...
    async def _resolve_and_hydrate_nodes_edges(
        ...
        # using only new edges to avoid duplication)
    ...
    async def _persist_episode_to_graph(
        ...
        episodic_edges = build_episodic_edges(
            resolved_nodes, episode.uuid, now, node_episode_index_map
        )
        episode.entity_edges = [edge.uuid for edge in all_entity_edges]
        # episode.content = ''  # Optional
        ...
        if update_communities:
            ...
            communities_result = await semaphore_gather(
                *[
